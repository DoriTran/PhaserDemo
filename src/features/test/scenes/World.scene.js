...
export class WorldScene extends Phaser.Scene {
  ...
  createEvents(events) {
    ...
    this.eventZones.addMultiple(
      events.map(
        ({ x, y, width, height, onEnter, onExit }) =>
          new EventZone(this, x, y, width, height, onEnter, onExit)
      ),
      true
    );
  ...
  createEncounters(encounters) {
    ...
    encounters.forEach((encounter) => {
      const sprites = encounter.sprites.map(
        ({ x, y, texture, frames, startFrameKey, startFlipX }) => {
